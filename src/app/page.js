...
export default function Home() {
        ...
        return entries.map((item) => {
            return (
                <div 
                    ...
                    className='mt-4 shadow-[#764838] shadow-md p-3 w-70 h-50'
                >
                    <p className='text-2xl font-serif text-[#764838]'>{item.name}</p>
                    <p className='font-normal'>{item.location}</p>
                    <p className='text-gray-500 italic font-extralight'>{item.note}</p>
                </div>
            )
        })
    }
    ...
    const renderDishes = (category) => {
        ...
        return  Object.entries(dishes).map((item) => {
            return (
                <div 
                    key={item[0]}
                    className={`h-10 text-center rounded-2xl mr-2 p-3 cursor-pointer shadow-[#9f2f20] shadow-sm text-xs font-normal 
                        hover:bg-[#FACA78] hover:text-[#9f2f20] 
                        ${currentDish == item[0] ? 'bg-[#FACA78]' : 'bg-[#9f2f20]'}
                        ${currentDish == item[0] ? 'text-[#9f2f20] ' : 'text-[#FACA78]'}`}
    ...
    const renderCategories = (db) => {
        return Object.entries(db).map((item) => {
            return (
                <div 
                    key={item[0]}
                    className={`font-elsie sm:w-50 md:w-50 lg:w-50 p-3 bg-[#9f2f20] text-base mb-3 cursor-pointer shadow-[#764838] shadow-md
                        hover:bg-[#FACA78] hover:text-[#9f2f20] 
                        ${currentCategory == item[0] ? 'bg-[#FACA78]' : 'bg-[#9f2f20]'}
                        ${currentCategory == item[0] ? 'text-[#9f2f20] ' : 'text-[#FACA78]'}`}
                    onClick={() => {
    ...
    return (
        <div className=''>
            <div className='flex justify-start flex-wrap pb-8 pt-12 pl-8 pr-8 sm:pl-24 md:pl-24 lg:pl-24 sm:pr-24 md:pr-24 lg:pr-24 gap-10 bg-[#9f2f20] w-screen sm:w-screen md:w-screen lg:w-screen h-20" border-[#764838] border-b-12'>
                <div className='max-w-xl'>
                    <Image src="/icon.jpg" width={200} height={200} alt="website-logo" className='object-scale-down'/>
                </div>

                <div className='flex flex-col'>
                    <h1 className='font-elsie font-bold text-8xl text-[#FACA78]'>
                        Anh Nguyen
                    </h1>
                    <h1 className='font-elsie font-bold text-8xl text-[#FACA78]'>
                        Food Review
